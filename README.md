...
[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.19731658.svg)](https://doi.org/10.5281/zenodo.19731658)
...
optimizable), supports varying-length referential values per attribute,
predicts one or several consequent attributes, allows rules to leave belief
unassigned as ignorance, and offers an optional JAX backend for JIT-compiled
inference and exact-gradient training via autodiff. The `BRBModel` class follows the
...
| Field                        | Description                                                    |
| ---------------------------- | -------------------------------------------------------------- |
| `output`                     | Scalar predictions, shape `(n_samples,)`, or `(n_samples, n_outputs)` |
| `utility_bounds`             | Lower and upper bound on `output` when the assessment is incomplete; equal to it when complete |
| `ignorance`                  | Belief the combination left unassigned, shape as `output`      |
...
**Referential values** are the discrete anchor points that define the input and output spaces. Each input attribute has its own set of referential values (which may differ in number), and each output has its own set of consequent referential values. The Cartesian product of all input referential values defines the set of rules.

**Several outputs** are declared by passing a list of consequent referential value arrays, one per output. Each output keeps its own grades, since objectives generally have their own scales and units. The activation weights depend only on the antecedents, so they are computed once and the combination runs once per output.
...
**Training** optimizes belief degrees, rule weights, attribute weights, and optionally the referential value positions themselves. All parameters are subject to constraints: each rule's belief degrees sum to 1 for every output, rule weights sum to 1, attribute weights are non-negative, and referential values remain sorted. Pass `allow_incomplete=True` to cap the belief sum at 1 instead of fixing it there, so training may leave a rule vague.
